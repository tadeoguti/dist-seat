//backend/src/services/checador/utils/Puppeteer/getDistribuidoraPuppeteer.js
...
async function getDistribuidoraPuppeteer(page, nameMarca, outputDir) {
  try {
    console.log(
      `Iniciando extracción de distribuidores para la marca: ${nameMarca}`,
    );
    ...
    await page.waitForSelector("#ContentPlaceHolder1_gvDatos", {
      timeout: 60000,
    });
    ...
    const datos = await page.evaluate(
      ...
      dominios_No_Deseados,
      nameMarca,
    );

    console.log(
      `✅ Total de Distribuidores encontrados en ${nameMarca}: ${datos.length}`,
    );

    if (datos.length === 0) {
      console.warn(
        `⚠️ No se encontraron distribuidores para la marca: ${nameMarca}`,
      );
    }
